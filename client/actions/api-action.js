...
export const getArticles = () => (dispatch, getState) => {
    ...
    return callApi(url, {
        ...
        dispatch({ type: 'GET_ARTICLES_ERROR', error })
    })
}
...
export const getArticlesId = () => (dispatch, getState) => {
    let id = parseId(getState().routing.locationBeforeTransitions.pathname)

    dispatch({ type: GET_ARTICLE_ID, id })
}
...
export const sendCommentForm = (name, email, text) => (dispatch, getState) => {
    dispatch({
